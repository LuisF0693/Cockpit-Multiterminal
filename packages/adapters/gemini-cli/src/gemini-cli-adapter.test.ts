...
import { GeminiCliAdapter, type GeminiPtyLike, type GeminiSpawnFn } from './gemini-cli-adapter';

function makeFakePty(pid = 999_960): GeminiPtyLike & {
  written: string[];
  killed: boolean;
...
function makeHarness(which: string | null = 'C:/npm/gemini.ps1'): {
  adapter: GeminiCliAdapter;
  ptys: Array<ReturnType<typeof makeFakePty>>;
} {
  const ptys: Array<ReturnType<typeof makeFakePty>> = [];
  const spawnFn: GeminiSpawnFn = () => {
    const fake = makeFakePty();
    ptys.push(fake);
    return fake;
  };
  return { adapter: new GeminiCliAdapter(spawnFn, () => which, 'gemini', 10), ptys };
}

const CONFIG = { cwd: 'C:/work', cols: 80, rows: 24 };

describe('GeminiCliAdapter (Story 12.4, FR39)', () => {
