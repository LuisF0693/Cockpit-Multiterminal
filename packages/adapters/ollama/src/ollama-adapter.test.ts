...
import { OllamaAdapter, type OllamaPtyLike, type OllamaSpawnFn } from './ollama-adapter';

function makeFakePty(pid = 999_980): OllamaPtyLike & {
  written: string[];
  killed: boolean;
...
function makeHarness(which: string | null = 'C:/Program Files/Ollama/ollama.exe'): {
  adapter: OllamaAdapter;
  ptys: Array<ReturnType<typeof makeFakePty>>;
} {
  const ptys: Array<ReturnType<typeof makeFakePty>> = [];
  const spawnFn: OllamaSpawnFn = () => {
    const fake = makeFakePty();
    ptys.push(fake);
    return fake;
  };
  return { adapter: new OllamaAdapter(spawnFn, () => which, 'ollama.exe', 10), ptys };
}

const CONFIG = { cwd: 'C:/work', cols: 80, rows: 24 };

describe('OllamaAdapter (Story 12.6)', () => {
  it('identidade output-parsing + availability com razão clara', async () => {
    ...
    expect(result.reason).toMatch(/ollama\.com/);
  });

  it('sem args explícito, spawna com o modelo default (run llama3)', async () => {
    const commands: Array<{ command: string; args: string[] }> = [];
    const spawnFn: OllamaSpawnFn = (command, args) => {
      commands.push({ command, args });
      return makeFakePty();
    };
    const adapter = new OllamaAdapter(spawnFn, () => 'C:/ollama.exe');
    await adapter.spawn(CONFIG);
    expect(commands).toEqual([{ command: 'ollama.exe', args: ['run', 'llama3'] }]);
  });

  it('com args explícito (modelo escolhido pelo usuário), usa exatamente esses args', async () => {
    const commands: string[][] = [];
    const spawnFn: OllamaSpawnFn = (_command, args) => {
      commands.push(args);
      return makeFakePty();
    };
    const adapter = new OllamaAdapter(spawnFn, () => 'C:/ollama.exe');
    await adapter.spawn({ ...CONFIG, args: ['run', 'mistral'] });
    expect(commands).toEqual([['run', 'mistral']]);
  });
