...
import { AntigravityAdapter, type AntigravityPtyLike, type AntigravitySpawnFn } from './antigravity-adapter';

function makeFakePty(pid = 999_970): AntigravityPtyLike & {
  written: string[];
  killed: boolean;
...
function makeHarness(which: string | null = 'C:/Users/dev/AppData/Local/agy/bin/agy.exe'): {
  adapter: AntigravityAdapter;
  ptys: Array<ReturnType<typeof makeFakePty>>;
} {
  const ptys: Array<ReturnType<typeof makeFakePty>> = [];
  const spawnFn: AntigravitySpawnFn = () => {
    const fake = makeFakePty();
    ptys.push(fake);
    return fake;
  };
  return { adapter: new AntigravityAdapter(spawnFn, () => which, 'agy', 10), ptys };
}
