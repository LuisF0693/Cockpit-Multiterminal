...
function defaultMaxTokens() {
  const env = parseInt(process.env.DESIGN_MD_MAX_TOKENS || "", 10);
  return Number.isFinite(env) && env > 0 ? env : DEFAULT_MAX_TOKENS;
}

// ── CLI args ────────────────────────────────────────────────────────
function parseArgs(argv) {
  const args = { url: null, out: null, prompt: null, compare: null, "no-content-gate": false, "no-llm-retry": false, "no-reuse": false, "no-bundle": false, "no-learning": false, "no-captures": false, "bundle-force": false, gallery: false, "max-cache-age": null, "max-llm-cache-age": null, "max-cost-usd": null, budget: null, provider: null, model: null, "max-tokens": null, scaffold: false, "scaffold-out": null, "scaffold-force": false, "emit-showcase": true, "emit-tailwind": false, "manual-recovery": false, mode: null, polish: null, source: null, project: null, resume: null };
  const positional = [];
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === "--no-bundle") args["no-bundle"] = true;
    else if (a === "--no-learning") args["no-learning"] = true;
    else if (a === "--no-captures") args["no-captures"] = true;
    else if (a === "--bundle-force") args["bundle-force"] = true;
    else if (a === "--gallery") args.gallery = true;
    else if (a === "--max-cache-age") {
      const n = parseFloat(argv[++i]);
      args["max-cache-age"] = Number.isFinite(n) && n >= 0 ? n : null;
    }
    else if (a === "--max-llm-cache-age") {
      const n = parseFloat(argv[++i]);
      args["max-llm-cache-age"] = Number.isFinite(n) && n >= 0 ? n : null;
    }
    else if (a === "--budget") args.budget = argv[++i] || null;
    else if (a === "--max-cost-usd") {
      const n = parseFloat(argv[++i]);
      args["max-cost-usd"] = Number.isFinite(n) && n > 0 ? n : null;
    }
    else if (a === "--provider") args.provider = argv[++i] || null;
    else if (a === "--model") args.model = argv[++i] || null;
    else if (a === "--max-tokens") {
      const n = parseInt(argv[++i], 10);
      args["max-tokens"] = Number.isFinite(n) && n > 0 ? n : defaultMaxTokens();
    }
    else if (a === "--scaffold") args.scaffold = true;
    else if (a === "--scaffold-out") args["scaffold-out"] = argv[++i] || null;
    else if (a === "--scaffold-force") args["scaffold-force"] = true;
    else if (a === "--emit-showcase") args["emit-showcase"] = true;
    else if (a === "--no-emit-showcase") args["emit-showcase"] = false;
    else if (a === "--emit-tailwind") {
      args["emit-tailwind"] = true;
      args["emit-showcase"] = true;
    }
    else if (a === "--manual-recovery") args["manual-recovery"] = true;
    else if (a === "--skip-llm") args["skip-llm"] = true;
    else if (a === "--mode") args.mode = argv[++i] || null;
    else if (a === "--polish") args.polish = argv[++i] || null;
  ...
  if (args["max-tokens"] == null) args["max-tokens"] = defaultMaxTokens();
  return args;
}
...
// "https://brand.aioxsquad.ai" → "aioxsquad"
...
function companyFromSlug(slug) {
  ...
  for (const t of COMPANY_TLDS) {
    const dashed = "-" + t.replace(/^\./, "").replace(/\./g, "-");
...
//   - distinct brand subdomain   → IS the company (e.g. redpine.aioxsquad.ai
...
//   https://brand.aioxsquad.ai/brandbook/guidelines → "aioxsquad-brand-brandbook-guidelines"
//   https://app.linear.app/                         → "linear-app"
//   https://docs.anthropic.com/en/api/messages      → "anthropic-docs-en-api-messages"
//   https://redpine.aioxsquad.ai/                   → "redpine"  (distinct brand)
...
function _hostParts(host) {
  ...
  // brand.aioxsquad.ai / app.linear.app / docs.anthropic.com →
  ...
  // e.g. redpine.aioxsquad.ai → company=redpine, no qualifier from parent
