...
const TASKS_DIR = path.join(ROOT, 'squads/design-system/tasks');
const CHECKLISTS_DIR = path.join(ROOT, 'squads/design-system/checklists');
...
function main() {
  if (!fs.existsSync(TASKS_DIR)) fail('Missing squads/design-system/tasks directory');
  if (!fs.existsSync(CHECKLISTS_DIR)) fail('Missing squads/design-system/checklists directory');
  ...
  for (const taskFile of taskFiles) {
    ...
    const refs = Array.from(
      sectionBody.matchAll(/`squads\/(?:design|design-system)\/checklists\/([^`]+\.md)`/g)
    ).map((m) => m[1]);
