...
function parseArgs(argv) {
  const args = { bu: null, app: null, format: 'json' };
...
function getWorkspaceConfig() {
  ...
  if (Array.isArray(raw.businesses)) {
    ...
    for (const entry of raw.businesses) {
      if (entry && entry.slug) obj[entry.slug] = entry;
    }
    raw.businesses = obj;
  }
  return raw;
...
function getConfigPath(businessSlug) {
  return getConfigCandidates(businessSlug).find((candidate) => fs.existsSync(candidate)) || getConfigCandidates(businessSlug)[0];
}
