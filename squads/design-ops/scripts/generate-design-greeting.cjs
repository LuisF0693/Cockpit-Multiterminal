...
const DEFAULT_SESSION_COMMAND = 'node squads/design-ops/scripts/set-active-context.cjs --business=default';
...
function buildContextSection(context, settings) {
  ...
  if (!context || !context.design_system) {
    return '## Active Design Context\n\nNo configured Design context is active.';
  }
...
function generateGreeting() {
  ...
  const intro = context
    ? 'Design Chief ativo. Orquestração canônica de Design Ops com contexto carregado do runtime da sessão.'
    : [
        'Design Chief ativo. Nenhum contexto de Design Ops foi inicializado nesta sessão.',
        '',
        '## Session Bootstrap',
        '',
