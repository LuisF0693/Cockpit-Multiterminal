...
function readSessionContext() {
  ...
  if (!fs.existsSync(sessionPath)) {
    fail(`Design session context not found: ${runtimePaths.toWorkspaceRelative(sessionPath)}. Run node squads/design-system/scripts/set-active-context.cjs --business=<slug> or --app=<id>.`);
  }
...
function buildOutput(args) {
  ...
  if (result.status === 'not_configured') {
    return {
      ...
      config_path: result.config_path,
      design_system: null,
      remediation: `Create ${result.config_path} or mark the BU as not_applicable in workspace/_system/config.yaml.`,
    };
  }
